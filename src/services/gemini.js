...
${selectedPlatforms.includes('naverBlog') ? `   - naverBlog: 네이버 블로그 글은 마크다운 기호(**, # 등) 및 HTML 태그는 절대로 포함하지 않는 대신, 가독성 향상과 구글 애드센스 품질 심사 통과를 위해 다음 지침을 반드시 엄격히 준수하십시오:
     * 각 소제목은 반드시 '📌 소제목' 또는 '【 소제목 】' 형태로 눈에 띄게 이모티콘을 조합해 단 1회 구성하십시오.
     * [🚨최우선 필수 금지령] 본문 중간의 소제목(제목)들을 제외한 **나머지 모든 본문 일반 서술 텍스트 내에는 어떠한 형태의 이모티콘이나 이모지(✨, ✔, 💡, 🧼, 🧽, 🔥, ⚡ 등)도 절대로 단 하나조차 사용하지 마십시오.** (본문 내 이모티콘 남발은 애드센스 품질 심사에 심각한 악영향을 주므로 본문 일반 텍스트 내 이모지 사용은 100% 전면 금지합니다.)
     * 한 문장은 2줄(PC 기준)을 넘지 않게 짧고 직관적으로 서술하며, 문맥의 호흡에 맞춰 2~3줄의 넉넉한 공백(여백) 라인을 단락 사이에 적극적으로 삽입하십시오.
