...
${selectedPlatforms.includes('naverBlog') ? `   - naverBlog: 네이버 블로그 글은 대부분 모바일로 소비되는 최신 트렌드를 엄격하게 반영하십시오. 마크다운 기호(**, # 등) 및 HTML 태그는 절대로 포함하지 않는 대신, 가독성과 시각적 가치(Aesthetics)를 극대화하기 위해 다음 지침을 반드시 따르십시오:
     * 각 소제목은 반드시 '📌 소제목' 또는 '【 소제목 】' 형태로 눈에 띄게 구성하십시오.
     * 핵심 강조 사항은 '✔ 강조 내용', '★ 필독:', '💡 핵심 요약:' 등 어울리는 이모지와 특수괄호를 조합하여 눈에 띄게 강조하십시오.
     * 한 문장은 2줄(PC 기준)을 넘지 않게 짧고 직관적으로 서술하며, 문맥의 호흡에 맞춰 2~3줄의 넉넉한 공백(여백) 라인을 단락 사이에 적극적으로 삽입하십시오.
     * 맥락과 어울리는 다채로운 이모지(🧼, 🧽, ✨, 🚿 등)를 매 3~4문장마다 적절히 배치하여 인플루언서 블로그 느낌이 나도록 작성하고, 제목 제안 3개와 해시태그를 포함하십시오.` : ''}
...
\`\`\`json
{
  "naverBlog": ${selectedPlatforms.includes('naverBlog') ? `{
    ...
    "content": "네이버 블로그 본문 (제목 제외, 문맥에 제휴 링크 및 하단 법적 고지 문구 포함. 절대 마크다운(**)이나 HTML 태그를 쓰지 않는 대신, 최신 모바일 가독성 트렌드에 따라 📌/【】/✔/💡 등의 이모지와 특수 기호로 소제목 및 핵심 구절을 강조하고, 문장 호흡에 맞춰 2~3줄의 풍부한 여백(빈 줄)을 적극적으로 삽입하여 가시성이 극대화된 매력적인 인플루언서 스타일 원고를 생성하십시오.)",
