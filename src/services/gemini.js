...
`,
    'aeo': `
[네이버 AI 브리핑 노출 대비 AEO(Answer Engine Optimization) 가이드라인]:
- AI 검색 엔진 및 AI 브리핑 시스템이 정보를 쉽게 추출(Parsing)하고 채택할 수 있는 명확한 3단계 계층형 구조로 작성하세요.
- 1단계: 도입부에 질문에 대한 명확한 정의/요약(두괄식 답변)을 1~2문장으로 제공하세요.
- 2단계: 핵심 정보를 한눈에 볼 수 있는 가로형 비교/분석 표(Table)를 텍스트 기반 마크다운 표 형식(예: | 항목 | 내용 |)으로 제공하되, 본문에 불필요한 마크다운 기호가 들어가지 않고 텍스트 형태로 표가 깔끔하게 렌더링되도록 하십시오.
- 3단계: 하단에 독자들이 궁금해할 만한 '자주 묻는 질문(FAQ)' 2~3가지를 일목요연하게 정리한 FAQ 블록을 포함하세요.
`,
    'home-plate': `
...
${selectedPlatforms.includes('naverBlog') ? `   - naverBlog: 네이버 블로그 글은 일반 마크다운 기호(**, # 등) 및 HTML 태그는 포함하지 않되, 제품 비교, 스펙 정리, 핵심 요약이 필요한 본문 중간 영역에는 텍스트 기반의 마크다운 표 형식(예: | 항목 | 내용 |)을 사용하여 구조화된 표를 적극적으로 작성하십시오. 또한 네이버 AI 로봇 저품질 필터를 무력화하고 가독성을 극대화하기 위해 다음 인간화 법칙을 100% 자동 적용하여 완벽하게 서술하십시오:
...
\`\`\`json
{
  "naverBlog": ${selectedPlatforms.includes('naverBlog') ? `{
    ...
    "content": "네이버 블로그 본문 (제목 제외, 문맥에 제휴 링크 및 하단 법적 고지 문구 포함. 일반 마크다운(**)이나 HTML 태그는 쓰지 않되, 요약/비교가 필요한 지점에는 마크다운 표 형식(| 항목 | 내용 |)을 적극적으로 사용하십시오. 소제목은 📌 또는 【】 등의 특수 괄호와 이모티콘으로 구성하고, 본문 텍스트 내에는 구글 애드센스 품질 심사 통과를 위해 어떠한 이모티콘이나 이모지도 절대 단 한 개도 쓰지 말아야 하며, 문장 호흡에 맞춰 2~3줄의 풍부한 여백(빈 줄)을 적극적으로 삽입하여 가시성이 극대화된 원고를 생성하십시오. 본문 중간에 적절한 문맥에 맞춰 이미지 및 동영상 가이드 표시인 [이미지 1: 설명], [동영상 1: 설명] 형태도 텍스트로 자연스럽게 이식해야 합니다.)",
